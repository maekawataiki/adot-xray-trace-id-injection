...
def log_hook(span, record):
    if span and span.is_recording():
        record.otelTraceID = convert_otel_trace_id_hex_to_xray(record.otelTraceID)
...
def convert_otel_trace_id_hex_to_xray(otel_trace_id_hex):
    return TRACE_ID_DELIMITER.join(
        [
            TRACE_ID_VERSION,
...
def convert_otel_trace_id_to_xray(otel_trace_id_decimal):
    ...
    x_ray_trace_id = convert_otel_trace_id_hex_to_xray(otel_trace_id_hex)
